...
class Quotes_model extends CI_Model
{
	...
	public function getQuoteItemsData($quote_id) {
		...
		$this->db->select('quote_items.id as id,quote_items.option_id, quote_items.product_code, quote_items.product_name, quote_items.quantity, quote_items.unit_price, quote_items.tax,quote_items.item_tax,quote_items.discount,quote_items.wpiece, quote_items.piece, quote_items.item_discount,quote_items.subtotal,tax_rates.name as taxs, quotes.*, IF(erp_quote_items.option_id, (SELECT name FROM erp_product_variants WHERE id = erp_quote_items.option_id),`erp_units`.`name`) AS `product_unit`,
						   products.id as product_id, product_variants.name as variant, products.image as image')
			->from('quote_items')
