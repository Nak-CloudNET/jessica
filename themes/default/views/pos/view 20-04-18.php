...
<div id="wrapper">
    ...
    <div id="receipt-data">
        <div class="text-center">
            ...
            ?>
			<!-- <div style="font-size:8px;">ហាងលក់សំលៀកបំពាក់ទារក-កុមារ និង ផលិតផលទារក-មាតា Online <br/>ដំបូងគេ និង ធំជាងគេ ដែលមានទីតាំងច្បាស់លាស់ប្រកបដោយទំនុកចិត្ត</div> -->
				...
				<table style="width: 100%;">				
					<tbody style="font-size: 10px;">						
						...
						<tr style="text-align:center;">	
							...
							<td style="vertical-align:middle;text-align:left;">វិក័យបត្រ</td>
							<td style="vertical-align:middle;text-align:right;"><?= $inv->reference_no ?></td>							
						</tr>
						<tr style="text-align:center;">	
							<td style="vertical-align:middle;text-align:left;padding:3px;">អាសយដ្ឋាន</td>							
							<td style="vertical-align:middle;text-align:left;"><?= $customer->address ?></td>						
						</tr>
						<?php if ($customer->phone) { ?>
							<tr style="text-align:center;">	
								<td style="vertical-align:middle;text-align:left;padding:3px;">ទូរស័ព្ទ</td>							
            ...
            <table class="table-condensed receipt no_border_btm" style="width:100%;">
				<thead>
					<tr style="border:1px dotted black !important;">
						<th>កូដ<br> Code</th>
						<th style="width: 30% !important">បរិយាយ <br> Description</th>
						<th style="text-align:center;">ចំនួន <br> Qty</th>
						<th style="text-align:right; width: 10% !important; text-align:center !important;">តម្លៃ <br> Price</th>
						<?php if ($inv->total_discount != $inv->order_discount) {
							echo '<th style="text-align:center !important;">បញ្ចុះ<br>'.'Dis.'.'</th>';
						} ?>
						<th style="padding-left:3px;padding-right:3px;text-align:center;">សរុប <br> Amount</th>
					</tr>
				</thead>
                ...
                <?php
					...
					foreach ($rows as $row) {
						...
						echo '<td class="text-left">' . $row->product_code . '</td>';
						...
						$colspan = 5;
						if ($inv->total_discount != $inv->order_discount) {
			...
			<table style="width: 100%; margin-top: 5px;">
				<tr>
					...
					<td style="text-align:right;width:35%; padding-right: 5px !important">Sub Total (<?= $default_currency->code; ?>) :</td>
					<?php if ($sub_total != $inv->grand_total) { ?>
						<td style="text-align:right;">$ <?=$this->erp->formatMoney($sub_total)?></td>
					<?php }else{ ?>
						<td style="text-align:right;font-size: 18px; font-weight: bold;">$ <?=$this->erp->formatMoney($sub_total)?></td>
					<?php } ?>
				</tr>
				<?php if ($inv->order_tax != 0) { ?>
				<tr>
					<td style="text-align:left;">ពន្ធ</td>
					<td style="text-align:right;width:35%; padding-right: 5px !important">Order Tax (<?= $default_currency->code; ?>) :</td>
				...
				<tr>
					...
					<td style="text-align:right;width:35%; padding-right: 5px !important">Discount (<?= $default_currency->code; ?>) :</td>
					<td style="text-align:right;">$ <?=$this->erp->formatMoney($inv->order_discount)?></td>
				</tr>
				<?php } ?>
				<?php if ($inv->shipping != 0) { ?>
				<tr>
					<td style="text-align:left;">ដឹកជញ្ជូន</td>
					<td style="text-align:right;width:35%; padding-right: 5px !important">Shipping (<?= $default_currency->code; ?>) :</td>
				...
				<tr>
					...
					<td style="text-align:right;width:40%; padding-right: 5px !important">Grand Total (<?= $default_currency->code; ?>) :</td>
			...
			<tr style="display:none;">
				...
				?>
					...
					<tr>
                        <th style="border-left:2px solid #000;border-top:2px solid #000;border-right:none;width: 35%"  class="text-right">ប្រាក់ទទួល / Received (<?= $default_currency->code; ?>) :</th>
                    ...
                    <tr>
                        <th style="border-left:2px solid #000;border-bottom:2px solid #000;border-right:none;width:64%;"  class="text-right">ប្រាក់ទទួល / Received (Riel) :</th>
                        <th style="border-bottom:2px solid #000;border-right:2px solid #000;border-left:none;" class="text-right"><?= $this->erp->formatMoney($inv->recieve_usd * $inv->other_cur_paid_rate) .' រ'; ?></th>
                    </tr>
				...
				<?php
					...
					<tr>
                        <th style="border-top:2px dotted #000;" class="text-right">ប្រាក់អាប់ / <?= lang("change_amount_us"); ?> :</th>
                        <th style="border-top:2px dotted #000" class="text-right">
						<?php
                    ...
                    <tr>
                        <th style="border-top:2px dotted #000;" class="text-right">ប្រាក់អាប់ / Change (Reil) :</th>
						<th style="border-top:2px dotted #000" class="text-right">
				...
				?>
					<tr>
                        <th style="border:2px solid #000;border-right:none;padding-right: 12px;" colspan="<?= $colspan ?>" class="text-right">ប្រាក់ទទួល / Received (<?= $default_currency->code; ?>):</th>
					...
					?>
                    <tr>
                        <th style="border:2px solid #000;border-right:none;padding-right: 12px;" colspan="<?= $colspan ?>" class="text-right">ប្រាក់ទទួល / Received (Riel):</th>
                        <th style="border:2px solid #000;border-left:none;" class="text-right"><?= $inv->other_cur_paid . '  ៛' ; ?></th>
                    </tr>
					<?php
						}else{}
				...
				?>
					<tr>
                        <th style="border-top:2px dotted #000;padding-right: 12px;" colspan="<?= $colspan ?>" class="text-right">ប្រាក់នៅខ្វះ / <?= lang("remaining_us"); ?> :</th>
                        <th style="border-top:2px dotted #000" class="text-right">
						<?php
                    ...
                    <tr>
                        <th style="border-top:2px dotted #000;padding-right: 12px;" colspan="<?= $colspan ?>" class="text-right">ប្រាក់នៅខ្វះ / Remaining (Reil) :</th>
				...
				<?php
                    ...
                    <tr>
                        <th style="border-top:2px dotted #000;padding-right: 12px; width: 62%" colspan="<?= $colspan ?>" class="text-right">ប្រាក់នៅខ្វះ / Remaining (Reil) :</th>
			...
			<div style="text-align:center;font-size:11px; margin-top: 5px">
					<?= nl2br($biller->invoice_footer); ?> <br>
				~ ~ ~ <b>www.jessicababyshop.com</b> &nbsp;&nbsp;&nbsp;&nbsp;<span style="font-size:12px;"></span> ~ ~ ~
			</div>
...
<?php } ?>
	<span class="pull-right col-xs-12">
        <a href="<?=base_url()?>pos/invoice_a5/<?=$sid?>" target="_blank" class="btn btn-block btn-primary" ><?= lang("invoice_a5"); ?></a>
    </span>
