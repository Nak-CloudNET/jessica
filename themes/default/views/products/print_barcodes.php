            ...
            <div class="col-lg-12">
                ...
                <div class="well well-sm no-print">
                    ...
                    <div class="controls table-controls">
                        ...
                        <div class="form-group">
                            <?= lang('style', 'style'); ?>
                            <?php $opts = array('' => lang('select').' '.lang('style'),40 => lang('40_per_sheet_jessica'), 65 => lang('A4 - 65 Labels per sheet')); ?>
                            <?= form_dropdown('style', $opts, set_value('style', 40), 'class="form-control tip" id="style" required="required"'); ?>
                                ...
                                foreach ($barcodes as $item) {
                                    ...
                                    for ($r = 1; $r <= $item['quantity']; $r++) {
                                        ...
                                        if($style == 6) {
                                            ...
                                            if($item['name']) {
                                                echo '<span class="barcode_name" style="font-size:14px">'.$item['name'].'</span>';
                                            }
                                            if($item['unit']) {
                                            ...
                                            if($item['price']) {
                                                echo '<span class="barcode_price" style="font-weight:bold; font-size:14px; display:block; padding-top:2 !important;margin-bottom:9px;">'.lang('$').' ';
                                                if($item['currencies']) {
                                                    foreach ($currencies as $currency) {
                                                        echo $currency->code . ': ' . $this->erp->formatMoney($item['price'] * $currency->rate).', ';
                                                    }
                                        ...
                                        if($item['name']) {
                                            echo '<span class="barcode_name" style="font-size:12px; margin-bottom:5px">'.$item['name'].'</span>';
                                        }
										if($item['image']) {
                                            echo '<span class="product_image" style="height:100%"><img  style="width:120px;height:126px" src="'.base_url('assets/uploads/thumbs/'.$item['image']).'" alt="" /></span>';
                                        }
                                        ...
                                        if($item['price']) {
                                            echo '<span class="barcode_price" style="font-weight:bold; font-size:11px !important; display:block; padding-top:2 !important;margin-bottom:9px;">'.lang('តម្លៃ $').' ';
                                            if($item['currencies']) {
                                                foreach ($currencies as $currency) {
                                                    echo $currency->code . ': ' . $this->erp->formatMoney($item['price'] * $currency->rate).', ';
                                                }
